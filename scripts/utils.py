...
def count_tokens(text) -> int:
    """Calculate number of tokens in a given input text using tiktoken tokenizer.

    Args:
        text (str): Input text

    Returns:
        int: Number of tokens calculated with tiktoken
    """
...
def get_filenames(
    ...
    if recursively:
        return sorted(list(inp_dir.glob(f"**/*.{ext.lower()}")))
    return sorted(list(inp_dir.glob(f"*.{ext.lower()}")))
...
def is_input_dir_valid(path_to_docs: Path) -> bool:
    ...
    if path_to_docs.exists():
        if path_to_docs.is_file() and path_to_docs.suffix.lower() == ".json":
            return True
        pdf_filenames = get_filenames(path_to_docs, "pdf", True)
        html_filenames = get_filenames(path_to_docs, "html", True)
        if len(pdf_filenames) != 0 and len(html_filenames) != 0 :
            return True
